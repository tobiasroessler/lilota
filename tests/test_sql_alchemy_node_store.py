...
class SqlAlchemyNodeStoreTestCase(TestCase):
    ...
    def test_update_status_on_dead_nodes___should_mark_old_nodes_as_dead(self):
        ...
        node1_id = self.create_node(
            Node(type=NodeType.WORKER, status=NodeStatus.IDLE, last_seen_at=old_time)
        )

        node2_id = self.create_node(
            Node(type=NodeType.WORKER, status=NodeStatus.IDLE, last_seen_at=recent_time)
        )
    ...
    def test_update_status_on_dead_nodes___should_not_update_already_dead(self):
        ...
        node_id = self.create_node(
            Node(type=NodeType.WORKER, status=NodeStatus.DEAD, last_seen_at=old_time)
        )
    ...
    def test_update_node_last_seen_at___should_update_timestamp(self):
        ...
        node_id = self.create_node(
            Node(type=NodeType.WORKER, status=NodeStatus.IDLE, last_seen_at=None)
        )
