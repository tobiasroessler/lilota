...
class LilotaSchedulerTestCase(TestCase):
    ...
    def test_start___with_heartbeat___should_start_the_heartbeat(self):
        # Arrange
        lilota = LilotaScheduler(
            LilotaSchedulerTestCase.DB_URL, node_heartbeat_interval=1.0
        )
        lilota.start()
    ...
    def test_logging___when_starting_scheduler___should_log_correctly(self):
        # Arrange
        lilota = LilotaScheduler(
            LilotaSchedulerTestCase.DB_URL, logging_level=logging.DEBUG
        )
    ...
    def test_logging___when_starting_scheduler_stopping_and_starting_it_again___should_log_correctly(
        self,
    ):
        # Arrange
        lilota = LilotaScheduler(
    ...
    def test_logging___when_starting_scheduler_twice___should_log_correctly(self):
        # Arrange
        lilota1 = LilotaScheduler(
            LilotaSchedulerTestCase.DB_URL, logging_level=logging.DEBUG
        )
        lilota2 = LilotaScheduler(
            LilotaSchedulerTestCase.DB_URL, logging_level=logging.DEBUG
        )
        log_store: LogStore = LogStore(LilotaSchedulerTestCase.DB_URL)
